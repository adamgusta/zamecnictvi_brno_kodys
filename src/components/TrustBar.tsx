      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 divide-y sm:divide-y-0 divide-x-0 sm:divide-x divide-slate-900">
          {items.map(({ icon: Icon, label, sub }) => (
            <div key={label} className="flex items-center gap-3 py-6 px-4 lg:px-6 hover:bg-slate-900/40 transition-all duration-300 group">
              <div className="w-10 h-10 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-115 group-hover:border-amber-500/40 transition-all duration-300">
                <Icon className="w-5.5 h-5.5 text-amber-400" />
              </div>
              <div>
                <p className="text-white font-extrabold text-sm tracking-tight">{label}</p>
